...
import { ArrowRight, BarChart2, Clock, Database, Delete, Trash2 } from "lucide-react"
...
export const DashboardPageContent = () => {
    const { data: categories, isPending: isEventCategoriesLoading } = useQuery({
        ...
        queryFn: async () => {
            const res = await client.category.getEventCategory.$get()
                ...
                <li key={category.id} className="relative group z-10 transition-all duration-200 hover:-translate-y-0.5">
                    ...
                    <div className="relative p-6 z-10">
                        ...
                        <div className="flex items-center justify-between mt-4">
                            ...
                            <Button variant="ghost" size="sm" className="text-gray-500 hover:text-red-600 transition-colors" aria-label={`Delete ${category.name}`}><Trash2 className="size-2"/></Button>
                        </div>
                    </div>
                </li>
            ))}
